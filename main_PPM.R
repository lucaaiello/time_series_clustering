...
sourceCpp('samplers/sampler_PPM.cpp')
...
init_M <- rgamma(1, shape = 1, rate = 1)
...
tic()
tseriesc_cohesion.out <- tseriesclust_cohesion_cpp(as.matrix(pollutant_scaled), as.matrix(locations), Z,
...
PPM_clusters <- tseriesc_cohesion.out$cluster

pollutant.out <- pollutant[ ,-tseriesc_cohesion.out$rem_obs]
pollutant_scaled.out <- pollutant_scaled[ ,-tseriesc_cohesion.out$rem_obs]
locations.out <- locations[-tseriesc_cohesion.out$rem_obs, ]

cohesion_clust_VI <- as.numeric(salso(tseriesc_cohesion.out$cluster, maxNClusters=0, loss=salso::VI(a=1), nRuns=100, nCores=6))

init_phi <- runif(length(unique(cohesion_clust_VI)))[cohesion_clust_VI] # 
init_tau2 <- 1/rgamma(length(unique(cohesion_clust_VI)), shape = 3, rate = 2)[cohesion_clust_VI] #

# rerunning the algorithm with partition obtained theough the VI for cluster specific estimates 

tic()
tseriesc_cohesion.out <- tseriesclust_cohesion_cpp(as.matrix(pollutant_scaled), as.matrix(locations), Z,
                                                   runs = 2000, burn = 1000, thin = 1, clust = FALSE,
                                                   init_beta = init_beta,
                                                   init_sigma2 = init_sigma2, init_sig2beta = init_sig2beta,
                                                   init_phi = init_phi, init_tau2 = init_tau2, init_M = init_M,
                                                   a_eps = 2, b_eps = 1,
...
phi <- apply(tseriesc_cohesion.out$phi, 2, median)
phi_m <- c()
tau2 <- apply(tseriesc_cohesion.out$tau2, 2, median)
tau2_m <- c()
for (i in 1:length(levels(as.factor(cohesion_clust_VI)))) {
  phi_m <- c(phi_m, round(mean(phi[which(cohesion_clust_VI==i)]),2))
  tau2_m <- c(tau2_m, round(mean(tau2[which(cohesion_clust_VI==i)]),2))
}

locations.clust <- data.frame(lon = locations.out$lon, lat = locations.out$lat, cluster = cohesion_clust_VI, phi = phi_m[cohesion_clust_VI], tau2 = tau2_m[cohesion_clust_VI])
locations.clust$gamma <- factor(paste("(",locations.clust$phi,",", locations.clust$tau2,")", sep = ""))

table(cohesion_clust_VI)

# Define a color palette with 9 colors
pal_colors <- colorRampPalette(c("green4", "green2", "yellow2", "orange2", "red2"))(9)

# Create a color factor using the generated palette
pal <- colorFactor(pal_colors, domain = levels(as.factor(locations.clust$gamma)), ordered = FALSE)
...
simm <- comp.psm(PPM_clusters)

heatmap(simm)

# Assuming simm_VI and z_VI are already created:
simm_VI <- simm[order(cohesion_clust_VI), order(cohesion_clust_VI)]
z_VI <- cohesion_clust_VI[order(cohesion_clust_VI)]
...
side_colors <- pal(factor(locations.clust$gamma))[order(cohesion_clust_VI)]
...
cluster_df <- data.frame(Series = colnames(pollutant.out), 
                         Cluster = factor(cohesion_clust_VI),
                         gamma = factor(paste("(",phi_m[cohesion_clust_VI],",", tau2_m[cohesion_clust_VI],")", sep = "")))  # Ensure it's a factor
